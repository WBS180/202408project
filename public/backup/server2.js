...
app.post('/harvest', (req, res) => {
    ...
    const pythonProcess = spawn('python', ['server.py', fruit, quantity]);
...
// HTML 페이지 라우팅
app.get('/', (req, res) => {
    //table은 harvest 와 fruit가 있다. FROM fruit.(tablename)
    connection.query('SELECT * FROM fruit.harvest', (err, results) => {
        if (err) throw err;
...
app.get('/harvest-data', (req, res) => {
    connection.query('SELECT fruit, SUM(quantity) as total FROM harvest GROUP BY fruit', (err, results) => {
        if (err) {
